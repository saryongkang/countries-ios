...
let defaultKeychainService = "com.example.keychain"
...
public struct KeychainAccess {
  ...
  public func get(forAccountKey account: String) -> String? {
    ...
    if readStatus != errSecSuccess {
      DDLogError("Failed to retrieve data for key: \(account). Reason: \(securityErrorMeesage(fromStatus: readStatus))")
      return nil
    }
    guard let passwordData = queryResult as? Data else {
      DDLogError("Failed to convert query result to data")
      return nil
    }
  ...
  public func exists(forAccountKey account: String) -> Bool {
    ...
    if readStatus != errSecItemNotFound {
      DDLogError("Failed to query existence of key: \(account). Reason: \(securityErrorMeesage(fromStatus: readStatus))")
    }
    return false
  }
